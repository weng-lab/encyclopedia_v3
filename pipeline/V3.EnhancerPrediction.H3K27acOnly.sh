...
#Pipeline for generating enhancer-like regions from H3K27ac peaks using H3K27ac signal
...
python ~/Projects/ENCODE/Scripts/prune.dnase.enhancer.py tmp-merge sorted-tmp | sort -k9,9n | awk '{print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" $9}' > $hset.EP

bedtools intersect -v -a $hset.EP -b $TSS > EP
bedtools intersect -u -a $hset.EP -b $TSS > P


awk '{print $1 "\t" $2 "\t" $3 "\t" "Distal-Prediction-"NR "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" "255,205,0"}' EP | head -n 20000 > $hset'_predictions.bed'
m=$(head -n 20000 EP | tail -1 | awk '{print $9}')
awk '{if ($9 < '$m') print $1 "\t" $2 "\t" $3 "\t" "Proximal-Prediction-"NR "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" "255,0,0"}' P >> $hset'_predictions.bed'
