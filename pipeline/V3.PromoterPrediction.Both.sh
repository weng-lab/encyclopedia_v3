...
sort -rgk8,8 z | awk '{print $0 "\t" NR}' | sort -rgk9,9 | awk '{print $0 "\t" NR}' | awk '{print $0 "\t" (0.28*$10+$11)/2}' | sort -gk12,12  | awk '{print $0 "\t" NR}'> Ranked-List
...
bedtools merge -i sorted4 > merge.bed
...
bedtools intersect -wo -f 1 -a no2 -b merge.bed > Forth
bedtools intersect -v -f 1 -a no2 -b merge.bed > no4

bedtools intersect -wo -a no4 -b merge.bed > Third
bedtools intersect -v -a no4 -b merge.bed > no3

awk '{print $14 "\t" $15 "\t" $16 "\t" $4 "\t" 1 "\t" "." "\t" $2 "\t" $3 "\t" $13}' First > tmp
awk '{print $14 "\t" $15 "\t" $16 "\t" $4 "\t" 1 "\t" "." "\t" $2 "\t" $3 "\t" $13}' Second >> tmp
awk '{print $14 "\t" $15 "\t" $16 "\t" $4 "\t" 1 "\t" "." "\t" $2 "\t" $3 "\t" $13}' Forth >> tmp
...
awk '{print $1 "\t" $2 "\t" $3 "\t" "Proximal-Prediction-"NR "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" "255,0,0"}' P | head -n 10000 > $dset'_'$hset'_predictions.bed'
m=$(head -n 10000 P | tail -1 | awk '{print $9}')
awk '{if ($9 < '$m') print $1 "\t" $2 "\t" $3 "\t" "Distal-Prediction-"NR "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" "255,205,0"}' EP >> $dset'_'$hset'_predictions.bed'
